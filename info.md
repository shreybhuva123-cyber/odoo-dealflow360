...
## 1. Problem Statement
DealFlow360 is an intelligent B2B Sales Operations and quotation orchestration platform designed for high-velocity enterprise B2B sales teams. It streamlines how sales teams create quotes, price products, apply discount rules, manage inventory fulfillment across multiple warehouses, and detect pricing anomalies. It automates quote approvals and multi-tier pricing so sales reps can close deals faster without manual back-and-forth delays or unauthorized discounts.

The **Customer Portal** is a high-trust, standalone external B2B experience (completely separated from internal ERP views) allowing enterprise clients, CFOs, and procurement leads to review proposals, inspect line items, negotiate terms and counter-discounts, compare revision diffs, post comments, and legally execute binding e-signatures under strict zero-leak and anti-IDOR security controls.
...
- **React Hook Form** – Performant form state and uncontrolled input management used in auth and configuration flows.
- **Zod** – TypeScript-first schema declaration and validation library enforcing credential and data constraints.
...
### Backend API & Database (Node.js & Express Platform)
- **Node.js** – JavaScript runtime environment executing backend server operations.
- **Express.js** – Web application framework defining API routes, controllers, middleware, and request/response pipelines.
- **PostgreSQL** – Relational database management system storing persistent data across 33 relational models and 21 enums.
- **Prisma ORM** – Type-safe ORM connecting Node.js with PostgreSQL with automated migrations and query building.
- **Zod** – Request validation middleware enforcing strict input contracts.
- **JWT (jsonwebtoken)** – Stateless Bearer token authentication with role claims.
- **bcryptjs** – Secure password hashing.
- **Morgan** – HTTP request logger.

### Customer Portal, Standalone Mock Server & Integration Layer
...
├── backend/
│   ├── src/
│   │   ├── config/                   # Central environment variable loading, cors, and risk constants
│   │   ├── controllers/              # 16 domain controllers (auth, quotes, approvals, billing, orders, etc.)
│   │   ├── routes/                   # REST routing definitions and authorization guards
│   │   ├── services/                 # Core business logic and database transaction services
│   │   ├── middleware/               # Auth token, RBAC, error, and logger middlewares
│   │   ├── validators/               # Zod input validation schemas
│   │   └── utils/                    # Shared utilities (jwt, password, pagination, apiResponse)
│   ├── prisma/
...
│   │   └── seed.js                   # Idempotent database seed pipeline
│   └── tests/                        # 14 comprehensive test suites (445 tests)
├── frontend/                         # Enterprise React 18 + TypeScript + Vite Single-Page Application
│   ├── src/
│   │   ├── components/               # Accessible UI primitives and module components
│   │   ├── pages/                    # 28+ lazy-loaded pages across all 12 modules
│   │   ├── services/                 # Authoritative API client and service endpoints
│   │   ├── stores/                   # Zustand client state stores
│   │   └── routes/                   # Route definitions and role guards
├── docs/                             # Architecture, API contract, and security specifications
├── mock_server/
...
│   ├── css/portal-foundation.css     # Core tokens, typography, CSS variables, and shimmer animations
│   ├── js/api/                       # Transport, TokenStore, QueryCache, ApiClient
│   ├── js/services/                  # Headless domain data-access services
│   ├── js/components/                # Pure presentational UI primitives
│   └── index.html                    # 14-screen interactive Customer Portal SPA
├── tests/                            # Automated Python test suites for Customer Portal
...
│   │   ├── assets/
│   │   ├── components/
│   │   │   ├── ui/                        # Accessible UI primitives (Button, Input, Dialog, Badge, Table, Tabs, etc.)
│   │   │   ├── layout/                    # AppShell, TopNavbar, Sidebar, Breadcrumbs, HackathonDemoTour, etc.
│   │   │   ├── navigation/                # ProtectedRoute, RoleGuard, PermissionGuard, nav-config
│   │   │   ├── dealflow/                  # QuotationBuilder, DiscountEditor, MarginIndicator, RiskIndicator, etc.
│   │   │   ├── approvals/                 # PriorityRiskBadge, RiskBreakdown, DiscountAnalysisTable, ApprovalActions
│   │   │   ├── pipeline/                  # PipelineBoard, PipelineColumn, DealCard, PipelineTable, DealOverview
│   │   │   ├── fulfillment/               # FulfillmentTable, WarehouseAllocation, CarrierSelector
│   │   │   ├── billing/                   # InvoiceTable, PaymentHistory, RecordPaymentDialog, SubscriptionCard
│   │   │   ├── notifications/             # NotificationBell, NotificationDropdown, NotificationFilters
│   │   │   ├── audit/                     # AuditLogTable, AuditFilters, AuditEventDetailsModal
│   │   │   ├── search/                    # CommandPalette, RecentSearches
│   │   │   └── feedback/                  # AttentionCenter, NetworkStatusBanner, ErrorBoundary, Skeletons
│   │   ├── pages/                         # 28+ lazy-loaded pages across all 12 modules
│   │   │   ├── auth/                      # LoginPage, SignupPage
│   │   │   ├── dashboard/                 # DashboardPage (KPI metrics, Attention Center, Velocity health)
│   │   │   ├── quotations/                # QuotationsListPage, QuotationCreatePage, QuotationDetailPage, etc.
│   │   │   ├── pipeline/                  # PipelinePage, DealDetailPage
│   │   │   ├── approvals/                 # ApprovalsPage, ApprovalDetailPage
│   │   │   ├── fulfillment/               # FulfillmentListPage, FulfillmentDetailPage
│   │   │   ├── billing/                   # InvoicesListPage, InvoiceDetailPage, BillingSchedulesPage
│   │   │   ├── subscriptions/             # SubscriptionsListPage, SubscriptionDetailPage
│   │   │   ├── customers/                 # CustomersListPage, CustomerDetailPage
│   │   │   ├── products/                  # ProductsListPage, ProductDetailPage, ProductFormPage
│   │   │   ├── warehouses/                # WarehousesListPage, WarehouseDetailPage
│   │   │   ├── deal-health/               # DealHealthDashboardPage, DealRiskDetailPage
│   │   │   ├── reports/                   # PipelineAnalyticsPage, RevenueForecastPage
│   │   │   ├── admin/                     # AdminOverviewPage, UsersPage, RolesPage, PricingRulesPage, SystemSettingsPage
│   │   │   ├── notifications/             # NotificationsPage
│   │   │   ├── audit-logs/                # AuditLogsPage
│   │   │   └── portal/                    # CustomerQuotePortalPage, CustomerNegotiationPage, CustomerConfirmationPage
│   │   ├── hooks/                         # useAuth, useQuotations, useApprovals, usePipeline, useNotifications, etc.
...
│   │   ├── routes/                        # route-config.tsx (code-split lazy imports), index.tsx
│   │   ├── App.tsx
│   │   └── main.tsx
│   ├── package.json
│   ├── tsconfig.json
...
│   ├── customer_portal_ui_design.md       # Complete 14-Screen B2B Customer Portal UI/UX Design System
...
│   ├── customer_portal_e2e_integration_test.md # End-to-End Integration Test Specification (22-Step Commercial Lifecycle)
│   └── customer_portal_ux_review_wow_moment.md # Senior SaaS UX Review, Polish Checklist & WOW MOMENT Live Demo
├── mock_server/
│   └── portal_mock_api.py                 # Standalone mock API & web server serving API and Customer Portal SPA
├── portal_ui/                             # Standalone Customer Portal Single-Page Application
│   ├── css/
...
│   ├── js/
│   │   ├── api/                           # Core transport, storage & RFC 7807 error primitives
│   │   │   ├── PortalApiError.js
│   │   │   ├── TokenStore.js
│   │   │   ├── QueryCache.js
│   │   │   └── ApiClient.js
│   │   ├── services/                      # Headless domain data-access services (Zero DOM coupling)
│   │   │   ├── AuthService.js, QuoteService.js, QuoteDetailService.js, NegotiationService.js,
│   │   │   │   CommentService.js, RevisionService.js, ConfirmationService.js, NotificationService.js, StatusService.js
│   │   │   └── index.js
│   │   ├── components/                    # Pure presentational UI primitives (badges, feedback, layout, overlays, etc.)
│   │   └── types/
│   │       └── portal-components.d.ts
│   └── index.html                         # Complete 14-screen interactive Customer Portal SPA
├── tests/                                 # Automated Python test suites (153/153 tests passing)
│   ├── test_portal_api_contract.py        # Contract tests (22 unit tests)
│   ├── test_portal_ui_screens.py          # UI screen & routing tests (16 unit tests)
│   ├── test_portal_security_auth.py       # Security, auth & anti-IDOR tests (18 unit tests)
│   ├── test_portal_foundation_components.py # Foundation components & static asset tests (16 unit tests)
│   ├── test_portal_quote_listing.py       # Quotation listing, search, sort, filter & zero-leak tests (14 unit tests)
│   ├── test_portal_quote_detail.py        # Quotation detail, line items, terms & e-sign tests (12 unit tests)
│   ├── test_portal_line_comments.py       # Line-level commenting, zero-leak & anti-IDOR tests (13 unit tests)
│   ├── test_portal_integration_layer.py   # Frontend integration layer & domain services tests (19 unit tests)
│   └── test_portal_e2e_integration.py     # End-to-end commercial lifecycle integration tests (23 unit tests)
